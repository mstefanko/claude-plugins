...
pass `--run-id`, use the work-order id stem as the explicit run id. If a
...
use date suffixes, do not rely on generated run ids for split execution, and do
not overwrite exact files unless the user explicitly asks.

After split approval, write all part files, validate all of them before running
any part, and then run the approved sequence or fanout. Route each part by its
own `type`: `build` uses `bakeoff build`; `gather`, `compare`, and `analyze`
use `bakeoff research`. Apply mode-specific flags to each part.
...
same preview rules before asking for fresh approval.
...
`0`, `3`, or `4`; exit `3` is a completed Bakeoff handoff with unresolved
disagreement, and exit `4` is a decision-incomplete handoff with durable
provider artifacts. Split runs continue after exit `4` because each part is
...
before spending more lens budget. Stop the sequence on exit `1`, `2`, `130`,
...
work-order-list schema, coordinate shared state across parts, create generic
`<out>/<base>.split-summary.md`, or synthesize an overall winner, merged patch,
or merged answer from split runs. Cross-run synthesis is a separate user
request. Multi-lens review has its own bounded summary rule below; generic
split runs remain independent and summaries use explicit run ids, never
`latest`.
...
For build fast-path drafts, get these fields by running `bakeoff draft-build`
with the extracted inputs and using its stdout JSON as the preview source.
Manual build JSON drafting is reserved for cases that need metric verifier
fields or careful path discovery.

Do not include `build.patch_max_bytes` in generated drafts. The Go CLI owns
that default; omit the field and let validation fill it.
