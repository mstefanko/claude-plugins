...
| Clean split | `/bakeoff:run compare our CLI setup flow against README expectations, and review my local diff for security` | Offers separate work orders only if each part has independent evidence; requires `split`; then requires the displayed split approval, validates all files, and runs nothing before every file validates. |
