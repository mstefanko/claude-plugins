...
To run multiple lenses, say so explicitly: `/bakeoff:run review this diff against main with security, performance, and UX as separate lenses`. Bakeoff previews one normal review run per lens; reply `write and run` or `sequential` to run them one after another, or `parallel` when the preview offers it to launch all 2-3 lens runs at once.
