...
argument-hint: "<plan-path>|<plan-path> --prepare --continue|--prepared <run-id> [--phase-sessions fanout|off|auto] [--codex-review auto|on|off] [--risk low|moderate|high] [--decompose=off|inspect|enforce] [--force-simple <phase_id>] [--force-decompose <phase_id>] [--auto]"
---
...
3. **Plan-prepare or prepared gate:** for normal plan paths, inspect the plan, optionally decompose each phase into a `work_units.v2` artifact, and create writer/spec-review child issues only after the artifact is accepted. For `--prepare --continue`, run `bin/swarm do <plan-path> --prepare --continue`; this defaults to controller-owned Agent fanout. Add `--phase-sessions off` only when you want the command to stop after prepare/verify, or `--phase-sessions auto` for the temporary legacy/debug path. It auto-accepts only clean deterministic artifacts, then runs the same prepared-dispatch validation. If it prints `Status: NEEDS_INPUT`, stop and have the operator run `/swarmdaddy:prepare --accept <run-id>` after review. For `--prepared`, validate the accepted artifact with `bin/swarm do --prepared <run-id>` before creating any Beads children; skip plan prepare and do not invoke `agent-decompose`. When phase sessions are enabled, `bin/swarm do ...` owns init/resume, reconciles stale or abandoned attempts before new claims, and runs the all-phases foreground pump; use `bin/swarm phases status <run-id>`, `bin/swarm phases recover <run-id> --json --dry-run`, and `bin/swarm context render --run-id <run-id> --phase <phase-id> --role <role>` for inspection and recovery.
