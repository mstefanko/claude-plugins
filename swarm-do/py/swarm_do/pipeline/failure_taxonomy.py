...
_SPECS = (
    FailureKindSpec(
        ...
        "retry",
        "Sub-agent timed out",
        "A dispatched sub-agent reported a timeout that can be retried with a fresh sub-agent.",
        ("child_result", "stage_controller"),
    ),
    FailureKindSpec(
        "RETRYABLE_RATE_LIMIT",
        "child_result",
        "retry",
        "Sub-agent hit a rate limit",
        "A dispatched sub-agent reported a transient provider rate limit.",
        ("child_result", "stage_controller"),
    ),
    FailureKindSpec(
        ...
        "child_result",
        "retry",
        "Sub-agent hit a transient network failure",
        "A dispatched sub-agent reported a network failure that can be retried.",
        ("child_result", "stage_controller"),
    ),
    FailureKindSpec(
        "NON_RETRYABLE_AUTH",
        "child_result",
        "human_gate",
        "Sub-agent authentication failed",
        "A dispatched sub-agent reported an authentication or authorization failure.",
        ("child_result", "stage_controller"),
    ),
    FailureKindSpec(
        ...
        "child_result",
        "human_gate",
        "Sub-agent input was invalid",
        "A dispatched sub-agent reported invalid input that needs operator or planner correction.",
        ("child_result", "stage_controller"),
    ),
    FailureKindSpec(
        ...
        "child_result",
        "human_gate",
        "Sub-agent capability is unsupported",
        "A dispatched sub-agent reported that the requested capability is unavailable.",
        ("child_result", "stage_controller"),
    ),
    FailureKindSpec(
        "NORMALIZATION_ERROR",
        "child_result",
        "human_gate",
        "Sub-agent result normalization failed",
        "A dispatched sub-agent result could not be normalized into the expected controller contract.",
        ("child_result", "stage_controller"),
    ),
    FailureKindSpec(
        "PARTIAL_SUCCESS",
        "child_result",
        "terminal",
        "Phase partially succeeded",
        "Some work units were adopted and some were blocked; preserve both sets for operator review.",
        ("stage_controller", "phase_result"),
    ),
    FailureKindSpec(
        "adoptable_artifacts",
        "artifact",
    ...
    FailureKindSpec(
        ...
        "permission",
        "human_gate",
        "Fanout dispatcher cannot call Agent",
        "Fanout mode was requested without bypass permissions or an Agent-inclusive allowlist.",
        ("command_metadata", "permission_contract_details"),
    ),
    FailureKindSpec(
        "dispatcher_token_exhausted",
        "launcher",
        "retry",
        "Dispatcher exhausted its token budget",
        "The dispatcher hit its token limit before producing a complete phase result.",
        ("stdout_or_outer_json", "stage_controller"),
    ),
    FailureKindSpec(
        "stage_result_missing",
        "artifact_contract",
        "human_gate",
        "Stage result was missing after marker",
        "The dispatcher emitted a stage completion marker but the referenced result JSON never appeared.",
    ...
    FailureKindSpec(
        ...
        "A dispatched sub-agent failed without a more specific retry taxonomy kind.",
        ("child_result", "stage_controller"),
    ),
    FailureKindSpec(
        ...
        "artifact",
        "human_gate",
        "Adoptable artifacts could not be committed",
        "The controller found valid stage artifacts but could not commit them into the execution worktree.",
        ("valid_result_artifact", "worktree_diff", "git_commit"),
    ),
    FailureKindSpec(
        "retry_cycle_cap_exceeded",
        "child_result",
        "human_gate",
        "Stage retry cycle cap exceeded",
        "A stage exhausted the controller's fresh-reviewer retry cycle cap.",
        ("stage_controller", "stage_session"),
    ),
    FailureKindSpec(
