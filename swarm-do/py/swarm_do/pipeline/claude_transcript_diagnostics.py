...
def load_transcript_diagnostics(
    ...
    *,
    ...
    if path is None:
        return _empty(session_id=session_id)
    return parse_transcript(path, session_id=session_id)
...
def parse_transcript(path: Path, *, session_id: str | None = None) -> TranscriptDiagnostics:
    ...
    return TranscriptDiagnostics(
        ...
        last_error_summary=_last_error_summary(tool_errors[-1]) if tool_errors else None,
    )
