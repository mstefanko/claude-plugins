...
from swarm_do.pipeline import execution_workspace, phase_pump
...
def test_claude_print_reports_ineligible_without_claiming_phase(monkeypatch: pytest.MonkeyPatch) -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        monkeypatch.setattr(phase_pump, "doctor_report", lambda: report)
...
def test_claude_print_injected_runner_completes_two_phases(fake_claude_on_path) -> None:
    with tempfile.TemporaryDirectory() as td:
...
def test_claude_print_forwards_legacy_max_budget(fake_claude_on_path) -> None:
    with tempfile.TemporaryDirectory() as td:
        repo, data, run_id = make_prepared_run(Path(td), phase_count=1)
...
def test_claude_print_uses_policy_attempt_budget_when_cli_budget_absent(fake_claude_on_path) -> None:
    with tempfile.TemporaryDirectory() as td:
        repo, data, run_id = make_prepared_run(Path(td), phase_count=1)
...
def test_claude_print_failed_phase_stops_downstream(fake_claude_on_path) -> None:
    with tempfile.TemporaryDirectory() as td:
        repo, data, run_id = make_prepared_run(Path(td), phase_count=2)
...
def test_claude_print_nonzero_without_valid_result_does_not_complete(fake_claude_on_path) -> None:
    with tempfile.TemporaryDirectory() as td:
        repo, data, run_id = make_prepared_run(Path(td), phase_count=1)
...
def test_claude_cli_missing_records_launch_dir_and_manifest(monkeypatch: pytest.MonkeyPatch) -> None:
    with tempfile.TemporaryDirectory() as td:
        repo, data, run_id = make_prepared_run(Path(td), phase_count=1)
        monkeypatch.setattr(phase_pump, "doctor_report", lambda: _eligible_claude_report())
        monkeypatch.setattr(phase_pump.shutil, "which", lambda name: None)
...
def test_claude_print_nonzero_complete_artifacts_are_adopted(fake_claude_on_path) -> None:
    with tempfile.TemporaryDirectory() as td:
        repo, data, run_id = make_prepared_run(Path(td), phase_count=1)
...
def test_claude_print_replayed_fixture_records_artifacts(fake_claude_on_path) -> None:
    with tempfile.TemporaryDirectory() as td:
        repo, data, run_id = make_prepared_run(Path(td), phase_count=1)
...
def test_claude_print_rewrites_sensitive_repo_paths_and_records_safe_cwd(fake_claude_on_path, monkeypatch: pytest.MonkeyPatch) -> None:
    with tempfile.TemporaryDirectory() as td:
        root = Path(td)
        fake_home = root / 'home'
        ...
        def runner(argv, prompt_text):
            ...
            return base_runner(argv, prompt_text)
        monkeypatch.setattr(execution_workspace.Path, "home", classmethod(lambda cls: fake_home))
        result = pump_phases(run_id, launcher='claude-print', max_phases=1, init_if_missing=True, claude_runner=runner, data_dir=data)
        assert result['status'] == 'complete'
        command = json.loads((data / 'runs' / run_id / 'phase_launches' / '1' / 'attempt-1' / 'command.json').read_text(encoding='utf-8'))
...
def test_claude_print_safe_cwd_can_be_disabled(fake_claude_on_path, monkeypatch: pytest.MonkeyPatch) -> None:
