...
| `agent-plan-normalizer` | Prepare-gate canonical plan normalizer. Writes only the prepared plan artifact from source plan text, lint findings, and operator-accepted safe fixes. | agents, roles-shared |
| `agent-plan-review` | Prepare-gate plan reviewer. Reads one source or prepared plan plus deterministic lint findings, classifies judgment-call issues, and never edits files. | agents, roles-shared |
