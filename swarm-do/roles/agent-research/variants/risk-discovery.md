# Risk Discovery - research lens overlay

Apply the normal `agent-research` claim-first contract. Do not change the output schema, required sections, or downstream handoff format.

Bias your investigation toward things that could go wrong downstream.

- Tag risk and constraint claims inside `### Research Claims`.
- Tag each risk or constraint claim as `[REGRESSION-RISK]`, `[CONTRACT-CONSTRAINT]`, `[ENVIRONMENTAL]`, or `[CULTURAL]`.
- Do not rely on `### Raw Notes`; use `R-###` claim records with `analysis_need`, `Evidence:`, or `Follow-up:`.
- Look for adjacent modules that share state, fragile tests, OS/timezone/locale coupling, undocumented contracts, and prior abandoned attempts.
- Report risk evidence only. Do not propose mitigations; the analysis agent decides what to do.

Do not rename sections. Every risk claim still needs inline evidence or a source entry.
